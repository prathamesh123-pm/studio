...
const CowIcon = ({ className }: { className?: string }) => (
  <svg 
    ...
    className={className}
  >
    {/* Professional Cow Silhouette SVG */}
  ...
  return (
    <nav className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container mx-auto px-2 md:px-4 flex h-16 items-center justify-between">
        <Link href="/" className="flex items-center gap-1.5 md:gap-2 font-bold text-base md:text-xl text-primary shrink-0">
          <div className="bg-primary/10 p-1.5 rounded-lg">
            <CowIcon className="h-6 w-6 md:h-7 md:w-7 text-primary" />
          </div>
          <span className="inline-block tracking-tight">Cattle Feed Survey App</span>
        </Link>
        <div className="flex items-center gap-0.5 md:gap-1 lg:gap-2">
          <Link href="/dashboard">
            <Button variant="ghost" size="sm" className="h-8 px-1.5 md:h-9 md:px-3 gap-1 md:gap-1.5 text-[10px] md:text-sm">
